...
function unlockAccount (target) {
  function unlockShowLoading(show) {
    $('#tfa-login').css('display', show ? 'none' : 'block');
    $('#tfa-loader').css('display', show ? 'block' : 'none');
  }
  unlockShowLoading(true);
  showError(false);
  $.ajax('/two-factor', {
    type: 'POST',
    data: {
  ...
  .done(function (data) {
    window.location = target || '/account';
  })
  .fail(function ( jqXHR, textStatus, errorThrown ) {
    unlockShowLoading(false);
    showError(true);
  });
}
