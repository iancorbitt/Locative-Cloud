...
exports.authorization = [
  ...
  server.authorization(function(clientID, redirectURI, done) {
    Client.findOne({clientId: clientID}, function(err, client) {
      ...
      console.log('[oauth2] required redirect_uri:', redirectURI);
      console.log('[oauth2] given redirect_uri:', client.redirectUri);
      if (redirectURI !== client.redirectUri) {
        let error = new Error('Redirect URI does not match!');
        error.status = 400;
        return done(error);
      }

      // installed applications flow / client credentials flow
